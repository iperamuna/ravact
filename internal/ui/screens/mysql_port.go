...
// MySQLPortModel represents the MySQL port change screen
type MySQLPortModel struct {
	theme   *theme.Theme
	width   int
	height  int
...
// NewMySQLPortModel creates a new MySQL port change model
func NewMySQLPortModel(manager *system.MySQLManager, config *system.MySQLConfig) MySQLPortModel {
...
func (m *MySQLPortModel) buildForm() *huh.Form {
	return huh.NewForm(
		huh.NewGroup(
			huh.NewInput().
				Title("New Port").
...
func (m MySQLPortModel) Init() tea.Cmd {
	return m.form.Init()
}

func (m MySQLPortModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	switch msg := msg.(type) {
...
func (m MySQLPortModel) View() string {
	if m.width == 0 {
		return "Loading..."
	}

	header := m.theme.Title.Render("Change MySQL Port")
